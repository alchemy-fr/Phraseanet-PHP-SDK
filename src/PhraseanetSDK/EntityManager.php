...
use PhraseanetSDK\VirtualProxy\VirtualProxyFactory;
...
class EntityManager
{
    ...
    public function __construct(APIGuzzleAdapter $adapter, array $options = array())
    {
        ...
        $proxyPath = isset($options['proxy.path']) ? $options['proxy.path'] : __DIR__ . '/../../cache/proxies';
        ...
        if (!$logger && $debug) {
            ...
            $logger->pushHandler(new StreamHandler(__DIR__ . '/../../log/doctrine.log'));
        }
